...
var SMTV_CLONE_REPO_URL = process.env.SMTV_CLONE_REPO_URL;
...
var getRepo = function (repoUrl, dirPath) { return __awaiter(_this, void 0, void 0, function () {
    var _a, _b, err, err2, stat, dotGitStat, dotGitPath, err3, pathArr, err4, err4;
        ...
        switch (_c.label) {
            ...
            case 2:
                ...
                if (err2 && err2.code !== 'ENOENT')
                    throw err;
                if (!(stat ? .isDirectory() && dotGitStat ? .isDirectory() :  : )) return [3 /*break*/, 4];
            ...
            case 3:
                ...
                return [3 /*break*/, 8];
            case 4:
                if (!(stat ? .isDirectory() && !dotGitStat ? .isDirectory() :  : )) return [3 /*break*/, 6];
...
var readFile = function (absolutePath) { return new Promise(function (resolve, reject) {
    fs_1["default"].readFile(absolutePath, 'utf8', function (err, text) {
        err && reject(err);
        !err && resolve({ filename: path_1["default"].basename(absolutePath), text: text });
    });
}); };
var parseVideoInfo = function (_a) {
    ...
    subTitle = subTitle ? .type === 'heading' && subTitle.depth === 2
        ...
        :
    ;
    var isDraft = tokenArr.links ? .draft ? .href === 'true'
        :
        :
    ;
    if (isDraft)
